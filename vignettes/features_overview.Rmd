...
The package consists of three functions that facilitate k-mer counting:
...
in following ways:
1. string vectors (e.g, `c("AAA", "ACDA")`)
2. list of string vectors (e.g., `list(c("A", "A", "A"), c("A", "C", "D", "A"))`)
...
The other function parameters should be `k = length(kmer_gaps)` (default)
and `positional = FALSE` (default).
...
This feature is particularly useful when a user wants to get information
related to presence or absence of k-mers. Then, they might set this feature
(`with_kmer_counts = FALSE`) and reduce the memory consumption.
...
The `[p_]` value is an integer that is used only in case of positional k-mers
...
The `s1.s2....sk` part represents consecutive elements of the k-mer.
Finally, the `g1.g2...gk-1` part represents the consecutive lengths of gaps.
